...
#   GPLv2+
#
#   Proofs for solving exponential demand model via analytic gradient
#   
#   Requires: minpack.lm
#
#   Purposes:
...
###
#  
#   Demand Function (Exponential form)
#
###
exponentialDemandFunc <- function(x, q0, alpha, k) 
{
  func <- (log(q0)/log(10)) + k * (exp(-alpha * q0 * x) - 1)
  eval(func)
}

###
#  
#   Jacobian of the demand function (Exponential form)
#
###
exponentialGradient <- function(x, q0, alpha, k) 
{
  func <- expression((log(q0)/log(10)) + k * (exp(-alpha * q0 * x) - 1))
  c(eval(D(func, "q0")), 
    eval(D(func, "alpha" )),
    eval(D(func, "k"  )))
}
...
#
###
exponentialResidualFunc <- function(params, x, consumption, demandFunction, jacobianFunction)
{
...
#
###
exponentialJacobianMatrix <- function(params, x, consumption, demandFunction, jacobianFunction) 
{
...
# Fitted base model
demandWithNoise <- do.call("exponentialDemandFunc", 
...
fit <- nls.lm(par = startingParameters, 
              fn = exponentialResidualFunc, 
              jac = exponentialJacobianMatrix,
              demandFunction = exponentialDemandFunc, 
              jacobianFunction = exponentialGradient,
              x = pricePoints, 
              consumption = log(simulatedConsumption), 
              control = nls.lm.control(maxiter = 1000,
                                       diag = c(q0 = 1, alpha = 1000, k = 1)))

# Map fitted params to base model
fittedDemand <- do.call("exponentialDemandFunc", 
                        c(list(x = pricePoints), 
                        fit$par))  
...
lines(pricePoints, exp(fittedDemand), col="red", lwd=1)   
